...
\subsection{Bước 3: Huấn luyện Mô hình BiLSTM (\texttt{train\_bilstm\_v3.py})}
 
Script \texttt{train\_bilstm\_v3.py} triển khai kiến trúc BiLSTM với Attention Mechanism, được thiết kế đặc biệt để xử lý dữ liệu chuỗi thời gian 2D với đầu vào có kích thước (64, 178):
 
\begin{itemize}
    \item \textbf{Linear Projection + LayerNorm + ReLU:} Chiếu dữ liệu đầu vào từ không gian 178 chiều lên 256 chiều, áp dụng chuẩn hóa lớp và hàm kích hoạt để ổn định quá trình huấn luyện. Đầu ra: (64, 256).
 
    \item \textbf{BiLSTM $\times$ 3 lớp:} Ba lớp BiLSTM với hidden dim = 256, bidirectional = True. Nhờ xử lý chuỗi theo cả hai chiều thuận và nghịch, mô hình có thể học các phụ thuộc ngữ cảnh dài hạn trong chuỗi ký hiệu. Đầu ra: (64, 512).
 
    \item \textbf{Attention Layer:} Cơ chế tự chú ý (self-attention) tính toán trọng số quan trọng của từng frame trong chuỗi, tổng hợp thành vector ngữ cảnh context có kích thước 512.
 
    \item \textbf{Concat [context, last\_h]:} Vector ngữ cảnh từ attention được nối với hidden state cuối cùng của BiLSTM, tạo ra vector tổng hợp 1024 chiều để đưa vào lớp phân loại.
\end{itemize}
 
Các siêu tham số của mô hình: Hidden dim = 256, LSTM layers = 3, Bidirectional = True, Attention = True, Dropout LSTM = 0.3, Dropout FC = 0.4.
...
\begin{itemize}
    \item \textbf{Stage 1 -- Gesture Model $\rightarrow$ Top-K = 3:} Mô hình BiLSTM đưa ra 3 từ ứng viên có xác suất cao nhất dựa trên đặc trưng cử chỉ tay.
