...
Bộ GD\&ĐT) và bộ dữ liệu tự thu thập thông qua công cụ \texttt{webcam\_collector.py}.
Dữ liệu tự thu thập được thiết kế theo cùng cấu trúc thư mục và định dạng của
QIPEDC nhằm đảm bảo tính nhất quán khi kết hợp hai nguồn. Bộ dữ liệu cuối cùng
bao gồm \textit{[X]} lớp ký hiệu với tổng cộng \textit{[X]} video gốc trước khi
tăng cường.
...
lệch lớp (class imbalance) trong quá trình huấn luyện.
...
tốt nhất \textbf{99.4\%} tại epoch 41. Khoảng cách nhỏ giữa đường train và val
cho thấy mô hình không bị overfitting, chiến lược dropout (0.2) và label
smoothing đã phát huy hiệu quả trong việc kiểm soát độ phức tạp của mô hình.
...
\begin{figure}[ht]
    ...
    \caption{Đường cong Loss và Accuracy trong quá trình huấn luyện
             (Train vs.\ Validation). Đường đứt nét cam đánh dấu epoch tốt nhất.}
...
Không có Emotion (201 dim) & \textit{[X]\%} & \textit{[X]\%}
    & \textit{[X]\%} & \textit{[X]\%} \\
\textbf{HandAware + Emotion (đề xuất, 208 dim)}
...
\begin{figure}[ht]
    ...
    \caption{Ma trận nhầm lẫn trên tập Test (chuẩn hóa theo hàng).
             Giá trị trên đường chéo chính biểu thị recall của từng lớp.}
    \label{fig:confusion}
\end{figure}

Để đánh giá chi tiết hơn theo từng lớp, Hình~\ref{fig:per_class} thể hiện
Precision, Recall và F1-score của từng ký hiệu riêng lẻ. Macro F1 đạt
\textbf{\textit{[X]\%}}, cho thấy mô hình hoạt động nhất quán trên tất cả các
lớp chứ không chỉ tốt ở các lớp có nhiều mẫu. Đường kẻ đứt nét trong biểu đồ
đánh dấu ngưỡng F1 = 80\% -- hầu hết các lớp đều vượt ngưỡng này, chỉ một số
ít lớp có F1 thấp hơn do đặc thù về độ phức tạp của ký hiệu.
...
\begin{figure}[ht]
    ...
    \caption{Precision, Recall và F1-score theo từng lớp ký hiệu VSL trên tập Test.
             Màu xanh lá: F1 $\geq$ 80\%; màu vàng: 50\% $\leq$ F1 $<$ 80\%;
             màu đỏ: F1 $<$ 50\%.}
    \label{fig:per_class}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Temporal Attention Weights -- mức độ chú ý của mô hình theo từng
             frame (màu đậm = được chú ý nhiều hơn). Mỗi hàng là một mẫu test
             với nhãn thật tương ứng.}
    \label{fig:attention}
\end{figure}
...
số thấp hơn. Hành vi này hoàn toàn phù hợp với đặc điểm tự nhiên của ngôn ngữ
ký hiệu: người thực hiện thường có một khoảng chuyển tiếp trước và sau mỗi ký
hiệu mà không mang thông tin ngữ nghĩa. Điều này cho thấy cơ chế Temporal
Attention Pooling đã học được đặc điểm quan trọng này một cách tự động, không
cần nhãn giám sát về vị trí bắt đầu và kết thúc ký hiệu.
...
\textbf{\textit{[X] điểm phần trăm}} (từ \textit{[X]\%} lên \textit{[X]\%}),
đặc biệt rõ ràng ở các cặp ký hiệu đồng hình -- những ký hiệu có cùng cấu
hình tay nhưng mang nghĩa khác nhau tùy theo biểu cảm khuôn mặt của người
thực hiện.

Ví dụ điển hình là cặp ký hiệu ``Cảm ơn'' và ``Ghét'': hai ký hiệu này có
cấu hình tay gần như giống hệt nhau, chỉ khác nhau ở biểu cảm khuôn mặt
(HAPPY/NEUTRAL so với ANGRY). Khi không có Emotion Encoding, mô hình nhầm
lẫn hai ký hiệu này với tần suất cao; khi bổ sung 7 chiều emotion, tỉ lệ nhận
dạng chính xác của cặp ký hiệu này tăng lên đáng kể.
